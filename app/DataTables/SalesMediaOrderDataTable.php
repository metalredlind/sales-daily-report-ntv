...
class SalesMediaOrderDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            $editBtn = "<a href='".route('sales.media-order.edit', $query->id)."' class='btn btn-info'><i class='far fa-edit'></i></a>";
            $deleteBtn = "<a href='".route('sales.media-order.destroy', $query->id)."' class='btn btn-danger ml-1 delete-item'><i class='fas fa-trash-alt'></i></a>";
    ...
    {
        return $this->builder()
                    ->setTableId('salesmediaorder-table')
                    ->columns($this->getColumns())
                    ->minifiedAjax(route('sales.media-order.data'))
                    //->dom('Bfrtip')
                    ->orderBy(0)
                    ->selectStyleSingle()
                    ->buttons([
    ...
    {
        return 'SalesMediaOrder_' . date('YmdHis');
    }
}
